 ...
 *   RFC 6120 §4.7.1 — 'id' attribute: MUST be generated by the server,
 ...
 *   RFC 6120 §4.7.4 — 'version' attribute MUST be "1.0"
 *     https://datatracker.ietf.org/doc/html/rfc6120#section-4.7.4
 *
 ...
 *   RFC 6120 §6.3.5 — SASL <mechanisms> advertisement
...
void handle_handshake_logic(xmpp_client_ctx_t *ctx) {
    ...
    /* RFC 6120 §4.7.1 — stream id: unique, unpredictable.
 ...
 *   "stream restart" requirement from RFC 6120 §6.3.6 step 4.
 * ------------------------------------------------------------------ */
void handle_sasl_success(xmpp_client_ctx_t *ctx) {
    /* RFC 6120 §6.3.6 — success element namespace is xmpp-sasl */
    ...
    /* RFC 6120 §6.3.6 step 5 — server resets its state for the stream */
    ctx->authenticated = 1;
 ...
 *   RFC 6120 §4.9.3.20 — <policy-violation/>: server SHOULD send a
...
err_t xmpp_recv_callback(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    if (ctx->rx_pos + (int)p->len < (int)(sizeof(ctx->rx_buffer) - 1)) {
         ...
         * TODO: RFC 6120 §4.9.3.20 — send <policy-violation/> stream error
    ...
    while (ctx->rx_pos > 0) {
         ...
         * RFC 6120 §6.3.6 — client MUST re-open after SASL <success/>.
