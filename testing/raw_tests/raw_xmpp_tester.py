...
Requirements:
    ...
    (optional: pip install pyopenssl  for TLS support)
"""
...
class RawXMPP:
    ...
    Exposes send/recv_until helpers.
    """
    ...
    def recv_until(self, pattern: str, timeout: float = RECV_TIMEOUT) -> str:
        """Accumulate bytes until pattern is found in decoded buffer or timeout."""
    ...
    def recv_all(self, wait: float = 1.0) -> str:
        """Accumulate all data received within `wait` seconds."""
    ...
    def upgrade_tls(self) -> bool:
        """Perform TLS upgrade after receiving <proceed/>."""
    ...
    def do_starttls(self) -> bool:
        """Send <starttls>, wait for <proceed/>, upgrade TLS, re-open stream."""
        self.send("<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'/>")
        resp = self.recv_until("<proceed", timeout=3)
        ...
        self.open_stream()
        resp = self.recv_until("<stream:features", timeout=3)
    ...
    def do_plain_auth(self, username: str, password: str) -> bool:
        ...
        resp = self.recv_until("</success>", timeout=3)
...
def test_rfc6120_stream_open_basic(host, port):
    ...
    try:
        c.connect()
        c.open_stream()
        resp = c.recv_until("<stream:features", timeout=3)

        has_stream = "stream:stream" in resp
        has_features = "stream:features" in resp
        has_from = f"from='{DOMAIN}'" in resp or f'from="{DOMAIN}"' in resp
        has_version = "version='1.0'" in resp or 'version="1.0"' in resp
        has_id = "id=" in resp

        record("stream:stream present in response", has_stream)
        record("stream:features present", has_features)
        record(f"from='{DOMAIN}' set (RFC 6120 §4.7.1)", has_from)
        record("version='1.0' set (RFC 6120 §4.7.5)", has_version)
        record("id= attribute present (RFC 6120 §4.7.3)", has_id)
...
def test_rfc6120_stream_open_wrong_to(host, port):
    """RFC 6120 §4.7.2 / §4.9.3.9 — Wrong 'to' → <host-unknown/>."""
    ...
    try:
        ...
        has_error = "<stream:error" in resp
        has_host_unknown = "host-unknown" in resp
        stream_closed = "</stream:stream>" in resp

        record("Stream error sent for wrong 'to' (RFC 6120 §4.9.3.9)", has_error,
               f"got: {resp[:200]}")
        record("<host-unknown/> condition present", has_host_unknown,
               f"got: {resp[:200]}")
        record("</stream:stream> sent before close", stream_closed,
               f"got: {resp[:200]}")
    except Exception as e:
        fail(f"Exception: {e}")
    finally:
...
def test_rfc6120_stream_close_graceful(host, port):
    """RFC 6120 §4.4 — Server sends </stream:stream> on clean client close."""
...
def test_rfc6120_sasl_plain(host, port):
    ...
    try:
        c.connect()
        c.open_stream()
        c.recv_until("<stream:features", timeout=3)
        ...
        resp2 = c.recv_until("success", timeout=3)
...
def test_rfc6120_iq_unknown_namespace(host, port):
    """RFC 6120 §8.2.3 — Unknown IQ get → <service-unavailable/>."""
    ...
    try:
        ...
        resp = c.recv_until("</iq>", timeout=3)
...
def test_rfc6121_roster_get(host, port):
    ...
    try:
        ...
        resp2 = c.recv_until("</iq>", timeout=3)
...
def test_rfc6121_roster_set_and_push(host, port):
    """RFC 6121 §2.1.5/6 — Roster set acknowledged, push sent."""
    ...
    try:
        ...
        resp2 = c.recv_until("</iq>", timeout=5)
        record("Roster get after set returns stored item",
               "user2@angelic.local" in resp2,
               f"got: {resp2[:400]}")
    except Exception as e:
        fail(f"Exception: {e}")
    finally:
...
def test_rfc6121_offline_message(host, port):
    ...
    try:
        ...
        body = "offline-test-message-12345"
        sender.send(f"<presence/>")
        sender.recv_all(wait=1)
        sender.send(
...
def test_rfc6121_subscription_flow(host, port):
    ...
    try:
        ...
        resp_a = alice.recv_all(wait=2.0)
        ...
        resp_roster = alice.recv_until("</iq>", timeout=3)
...
def test_xep0045_room_create_join(host, port):
    ...
    try:
        ...
        c.send(
            ...
            f"<field var='FORM_TYPE'><value>http://jabber.org/protocol/muc#roomconfig</value></field>"
            f"</x>"
            f"</query>"
            f"</iq>"
        )
...
def test_xep0045_nick_conflict(host, port):
    """XEP-0045 §7.2.8 — Nick conflict → <conflict/> error."""
...
def test_xep0045_groupchat_message(host, port):
    ...
    try:
        ...
        c1.send(f"<presence to='{room}/alice'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c1.recv_all(wait=1)
        c1.send(f"<iq type='set' to='{room}' id='cr2'><query xmlns='http://jabber.org/protocol/muc#owner'><x xmlns='jabber:x:data' type='submit'><field var='FORM_TYPE'><value>http://jabber.org/protocol/muc#roomconfig</value></field></x></query></iq>")
        c1.recv_all(wait=1)

        c2.send(f"<presence to='{room}/bob'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c2.recv_all(wait=1)

        body = "hello-groupchat-test-12345"
        c1.send(f"<message to='{room}' type='groupchat' id='gc1'><body>{body}</body></message>")

        resp1 = c1.recv_all(wait=2.0)
        resp2 = c2.recv_all(wait=2.0)

        record("Groupchat message reflected to sender (XEP-0045 §7.9)",
               body in resp1,
               f"sender got: {resp1[:400]}")
        record("Groupchat message delivered to other occupant (XEP-0045 §7.9)",
               body in resp2,
               f"other got: {resp2[:400]}")
...
def test_xep0045_private_message(host, port):
    ...
    try:
        ...
        c1.send(f"<presence to='{room}/alice'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c1.recv_all(wait=1)
        c1.send(f"<iq type='set' to='{room}' id='cr3'><query xmlns='http://jabber.org/protocol/muc#owner'><x xmlns='jabber:x:data' type='submit'><field var='FORM_TYPE'><value>http://jabber.org/protocol/muc#roomconfig</value></field></x></query></iq>")
        c1.recv_all(wait=1)
        c2.send(f"<presence to='{room}/bob'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c2.recv_all(wait=1)
        c3.send(f"<presence to='{room}/charlie'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c3.recv_all(wait=1)

        secret = "private-msg-only-for-bob-9876"
        c1.send(
        ...
        record("Private message NOT delivered to other occupants (XEP-0045 §7.13)",
               secret not in resp3,
               f"charlie got: {resp3[:400]}")

        for cx in (c1, c2, c3):
            try:
                cx.send(f"<presence to='{room}/alice' type='unavailable'/>")
...
def test_xep0045_nick_change(host, port):
    """XEP-0045 §7.6 — Nick change (not a new join)."""
    ...
    try:
        ...
        c1.send(f"<presence to='{room}/alice'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c1.recv_all(wait=1)
        c1.send(f"<iq type='set' to='{room}' id='cr4'><query xmlns='http://jabber.org/protocol/muc#owner'><x xmlns='jabber:x:data' type='submit'><field var='FORM_TYPE'><value>http://jabber.org/protocol/muc#roomconfig</value></field></x></query></iq>")
        c1.recv_all(wait=1)
        c2.send(f"<presence to='{room}/bob'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c2.recv_all(wait=1)

        c1.send(f"<presence to='{room}/alice2'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        resp1 = c1.recv_all(wait=2.0)
        resp2 = c2.recv_all(wait=2.0)
        ...
        c1.send(f"<presence to='{room}/alice2' type='unavailable'/>"); c1.recv_all(wait=0.5)
        c2.send(f"<presence to='{room}/bob' type='unavailable'/>"); c2.recv_all(wait=0.5)
    except Exception as e:
        fail(f"Exception: {e}")
    finally:
        c1.close(); c2.close()
...
def test_xep0045_occupant_presence_update(host, port):
    ...
    try:
        ...
        c1.send(f"<presence to='{room}/alice'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c1.recv_all(wait=1)
        c1.send(f"<iq type='set' to='{room}' id='cr5'><query xmlns='http://jabber.org/protocol/muc#owner'><x xmlns='jabber:x:data' type='submit'><field var='FORM_TYPE'><value>http://jabber.org/protocol/muc#roomconfig</value></field></x></query></iq>")
        c1.recv_all(wait=1)
        c2.send(f"<presence to='{room}/bob'><x xmlns='http://jabber.org/protocol/muc'/></presence>")
        c2.recv_all(wait=1)
        ...
        c1.send(f"<presence to='{room}/alice' type='unavailable'/>"); c1.recv_all(wait=0.5)
        c2.send(f"<presence to='{room}/bob' type='unavailable'/>"); c2.recv_all(wait=0.5)
    except Exception as e:
        fail(f"Exception: {e}")
    finally:
...
def test_disco_info_server(host, port):
    ...
    try:
        ...
        record("disco#info → MUC feature advertised",
               "jabber.org/protocol/muc" in resp,
               f"got: {resp[:400]}")
...
def main():
    parser = argparse.ArgumentParser(description="XMPP protocol compliance test harness")
