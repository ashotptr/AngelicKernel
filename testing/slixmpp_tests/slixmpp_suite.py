...
class TestClient(ClientXMPP):
    ...
    async def wait_for_message(self, body_contains: str, timeout: float = 5.0) -> Optional[slixmpp.Message]:
...
async def test_connect_and_session(host, port):
    ...
    try:
        ...
        if ok_flag:
            record("Bound JID has resource",
                   "/" in str(c.boundjid.full),
                   str(c.boundjid.full))
    finally:
        await c.stop()
...
async def test_roster_update_and_subscription(host, port):
    ...
    try:
        ...
        try:
            iq = await alice.update_roster(bare_bob, name="Bob")
        ...
        record("Bob receives subscription request (RFC 6121 §3.1.3)",
               sub_received,
               f"Bob presence: {[str(p['type']) for p in bob.received_presence]}")
        ...
        record("Alice receives subscribed confirmation (RFC 6121 §3.1.3)",
               sub_ack,
               f"Alice presence: {[str(p['type']) for p in alice.received_presence]}")

        await alice.get_roster()
        roster_item = alice.client_roster.get(bare_bob)
        if roster_item:
            sub_state = roster_item["subscription"]
            record("Alice's roster shows subscription='to' after subscribed (RFC 6121 §3.1)",
...
async def test_muc_join_and_chat(host, port):
    ...
    try:
        ...
        alice.clear_received()
        await alice_muc.join_muc_wait(room_jid, "alice", maxwait=5)
        record("Alice joins MUC room (XEP-0045 §7.2)", True)

        try:
            await alice_muc.set_affiliation(room_jid, alice.boundjid.bare, "owner")
        except Exception:
            pass
            
        bob.clear_received()
        await bob_muc.join_muc_wait(room_jid, "bob", maxwait=5)
        ...
        record("Groupchat message received by other occupant (XEP-0045 §7.9)",
               bob_got,
               f"Bob messages: {[str(m['body']) for m in bob.received_messages]}")
        ...
        record("Groupchat message reflected to sender (XEP-0045 §7.9)",
               alice_got,
               f"Alice messages: {[str(m['body']) for m in alice.received_messages]}")
...
async def test_muc_private_message(host, port):
    ...
    try:
        ...
        room_jid = f"privateroom@{MUC_SVC}"
        alice_muc = alice["xep_0045"]
        bob_muc   = bob["xep_0045"]
        ...
        await alice_muc.join_muc_wait(room_jid, "alice", maxwait=5)
        await bob_muc.join_muc_wait(room_jid, "bob", maxwait=5)
        await charlie_muc.join_muc_wait(room_jid, "charlie", maxwait=5)
        await asyncio.sleep(0.5)
...
async def test_disco(host, port):
    ...
    try:
        ...
        try:
            ...
            has_muc_item = any(
                "conference" in str(item) for item in items["items"]
            )
            record("disco#items on server lists MUC service (XEP-0030 §3.2)",
                   has_muc_item,
                   str(list(items["items"])[:3]))
        except Exception as e:
